...
**Visual-direction update (July 2026): low-poly 3D on a fixed isometric camera (2.5D), not hand-illustrated 2D sprites.** The diorama and characters are rendered from low-poly 3D models viewed through a fixed isometric camera, superseding the original "hand-illustrated 2D storybook" plan (§5). *Why:* ready-made 3D kits give a cohesive, extensible town far faster than commissioning 2D art, and Godot handles low-poly 3D well. **Current packs:** the town uses **Kenney's Starter Kit: City Builder** (MIT kit / CC0 models — clean 1×1-grid buildings, roads, grass, tree tiles, a fountain, one shared colormap); characters use **KayKit *Adventurers*** (CC0) as a placeholder. *(An earlier KayKit City Builder Bits pack was trialled for the town and dropped in favour of the cleaner Kenney kit.)* **What does not change:** the hub-and-card interaction model, one-tap-deep navigation, the single-screen fixed diorama (still deliberately not a scrollable open map), the storybook *palette* and warmth, and the one-decision-per-screen UX laws — only the render pipeline is 3D. The `gl_compatibility` renderer (OpenGL ES 3 / WebGL 2) is retained deliberately: it is the most compatible target for the HTML5 web export (fun-test) and low-end mobile, and low-poly scenes run comfortably on it. We use the kits' **models only**, not their builder/GridMap mechanics; the town is composed procedurally. **Placeholder caveat:** the *Adventurers* pack is medieval-fantasy with weapons (knights, swords, mages) — it conflicts with the modern, kid-safe setting and its professions (Baker, Vet, Coder, Teacher, Shopkeeper, Builder; spec §5) and is used only as a temporary stand-in. A modern, profession-appropriate, weapon-free character set must replace it before any external playtest that judges art.
