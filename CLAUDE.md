...
- **Art direction:** *(design doc §18 visual-direction update, July 2026)* **low-poly 3D on a fixed isometric camera (2.5D)**, superseding the earlier hand-illustrated-2D plan. Town built from **Kenney's Starter Kit: City Builder** (MIT/CC0 — 1×1-grid buildings, roads, grass, tree tiles, fountain, shared colormap); characters from **KayKit Adventurers** (CC0, **placeholder** — weapon-bearing medieval-fantasy, must be replaced with a modern, weapon-free set before any art-judged playtest). We use the kits' models only, not their builder mechanics; the town is composed procedurally in [game/scripts/diorama.gd](game/scripts/diorama.gd). Assets live in [game/assets/](game/assets/). The storybook *palette* and warmth carry onto the 3D frame; saturation dials up for Band A, down toward Band C. Key UI palette: canvas #F7F6F2, sky #CBE6EF, coin gold #E7B24C, alert red #E24B4A, growth green #7FB77F, warm text browns #7A5410/#2C2C2A.
